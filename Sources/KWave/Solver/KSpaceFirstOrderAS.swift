...
/// source; binary sensor mask recording `p`/`pFinal`. Time-varying sources, absorption, and
/// nonlinearity follow in later slices.
public func kspaceFirstOrderAS(
    ...
    precondition(source.p0 != nil, "this slice supports an initial-pressure (p0) source only")
    ...
    var p0Field = source.p0!.asType(.float32)
    if options.smoothP0 { p0Field = smooth(p0Field) }
    ...
    for t in 0..<grid.nt {
        ...
        // Mass conservation and the (lossless) equation of state.
        ...
        if t == 0 {
            p = p0Field
            rhox = p0Field / (2 * c2Grid)
            rhoy = p0Field / (2 * c2Grid)
