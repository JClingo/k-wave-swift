...
3. 🚧 `KWaveArray` (off-grid transducer arrays): ✅ `offGridPoints` (exact + truncated `tolStar` BLI), `KWaveArray` 2D with arc/line/rect elements — `elementGridWeights`/`arrayGridWeights`/`arrayBinaryMask`/`distributedSourceSignal`/`combineSensorData` (C order; `Array/KWaveArray.swift`; parity-tested — full off-grid source/sensor loop) · ❌ disc/bowl/annulus elements (need `makeCartDisc`/`makeCartBowl`), affine array transforms
