...
3. ✅ `KWaveArray` (off-grid transducer arrays): `offGridPoints` (exact + truncated `tolStar` BLI); arc/line/rect/disc (2D) + disc/focused-bowl (3D) elements; `elementGridWeights`/`arrayGridWeights`/`arrayBinaryMask`/`distributedSourceSignal`/`combineSensorData` (C order; `Array/KWaveArray.swift`; parity-tested incl. the full off-grid source/sensor loop). Not implemented: annulus elements, affine array transforms, axisymmetric weights branch
