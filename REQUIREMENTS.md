...
3. 🚧 `KWaveArray` (off-grid transducer arrays): ✅ `offGridPoints` — exact BLI + truncated `tolStar` path (`bliTolerance`, k-Wave's default; ported from MATLAB `tolStar.m`; `Array/OffGridPoints.swift`; parity-tested 2D/3D) · ❌ element integration, `getArrayGridWeights`
