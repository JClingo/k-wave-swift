...
1. 🚧 Axisymmetric solver: ✅ `kspaceFirstOrderAS` WSWA-FFT variant (`Solver/KSpaceFirstOrderAS.swift`) — linear, lossless, scalar/varying c0/rho0, p0 + time-varying pressure sources (dirichlet/additive/additive-no-correction), one-sided radial PML, cylindrical `uy/r` term, Stokes absorption, B/A nonlinearity; parity vs an independent NumPy port of MATLAB's WSWA-FFT branch (the macOS C++ binary aborts on AS inputs — FFTW lacks real-to-real). Plus velocity sources (3 modes; ux WSWA / uy HAHS correction) and the DTT foundation (`dct`/`dst` I-IV, `makeDTTDim`; `FFT/DTT.swift`) for the future DTT-variant · ❌ WSWS symmetry, DTT (non-FFT) variant
2. 🚧 CW propagation: ✅ `angularSpectrumCW` + broadband `angularSpectrum` incl. power-law absorption (Eq. 11; the upstream Python absorbing branch is dead code — Swift implements it correctly), angular restriction, grid expansion, time series (`Solver/AngularSpectrum.swift`; parity-tested) + `acousticFieldPropagator` (exact CW Green's-function propagator, ramped + unramped, 1D/2D/3D; `Solver/AcousticFieldPropagator.swift`; parity vs an independent NumPy port of the MATLAB formulas) + broadband reverse projection — CW propagation **complete**
3. ✅ `KWaveArray` (off-grid transducer arrays): `offGridPoints` (exact + truncated `tolStar` BLI); arc/line/rect/disc (2D) + disc/focused-bowl/annulus/annular-array (3D) elements; `elementGridWeights`/`arrayGridWeights`/`arrayBinaryMask`/`distributedSourceSignal`/`combineSensorData` (C order; `Array/KWaveArray.swift`; parity-tested incl. the full off-grid source/sensor loop and a 4-element annular array). Plus affine array transforms (`setArrayPosition`/`setAffineTransform`/`makeAffine`, 2D + 3D Euler; parity-tested). Not implemented: axisymmetric weights branch, `numPointsInner` contiguous-array alignment
4. ✅ `KWaveTransducer` (linear-array model; `Transducer/KWaveTransducer.swift`): geometry, element masks, azimuth (focus+steering) + elevation beamforming delays, delay mask, Hanning/custom apodization, input-signal padding, receive combination + scan-line beamforming — parity-tested vs `NotATransducer`. Not implemented: steering_angle_max offset mode, expand/retract grid
...
7. ✅ Reconstruction: `kspaceLineRecon` (2D) + `kspacePlaneRecon` (3D), nearest interp; `Reconstruction/`; parity-tested
...
9. ✅ Material properties: `waterSoundSpeed`, `waterDensity`, `waterAbsorption`, `waterNonlinearity` (`Material/WaterProperties.swift`), `powerLawKramersKronig`, `fitPowerLawParams` (faithful scipy-`fmin` Nelder-Mead port; `Material/PowerLaw.swift`) — all parity-tested
