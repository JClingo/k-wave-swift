...
3. 🚧 `KWaveArray` (off-grid transducer arrays): ✅ `offGridPoints` (exact + truncated `tolStar` BLI) and `KWaveArray` 2D with arc/line/rect elements — `elementGridWeights`/`arrayGridWeights` (`Array/KWaveArray.swift`; parity-tested) · ❌ disc/bowl/annulus elements (need `makeCartDisc`/`makeCartBowl`), `getDistributedSourceSignal`, `combineSensorData`, affine array transforms
