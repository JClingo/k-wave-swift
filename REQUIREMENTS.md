...
1. 🚧 Axisymmetric solver: ✅ `kspaceFirstOrderAS` WSWA-FFT variant (`Solver/KSpaceFirstOrderAS.swift`) — linear, lossless, scalar/varying c0/rho0, p0 + time-varying pressure sources (dirichlet/additive/additive-no-correction), one-sided radial PML, cylindrical `uy/r` term; parity vs an independent NumPy port of MATLAB's WSWA-FFT branch (the macOS C++ binary aborts on AS inputs — FFTW lacks real-to-real). Plus DTT foundation (`dct`/`dst` I-IV, `makeDTTDim`; `FFT/DTT.swift`) for the future DTT-variant · ❌ velocity sources, absorption (Stokes), nonlinearity, WSWS symmetry, DTT (non-FFT) variant
